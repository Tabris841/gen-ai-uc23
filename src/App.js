...
const TITLE_HEADERS = ['id', 'title', 'description', 'release_year', 'age_certification', 'runtime', 'genres', 'production_country', 'seasons'];
const CREDIT_HEADERS = ['id', 'title_id', 'real_name', 'character_name', 'role']
...
function App() {
  ...
  function generateData() {
    const titles = [...Array(100)].map((_, i) => ({
      ...
      release_year: faker.date.between({ from: '1900-01-01', to:new Date()}).getFullYear(),
      age_certification: getRandomElement(AGE_CERTIFICATION),
      runtime: getRandomNumberInRange(80, 200),
      genres: [...Array(getRandomNumberInRange(1, 3))].map(() => getRandomElement(GENRES)),
      production_country: faker.location.countryCode(),
      seasons: Math.random() > 0.7 ? getRandomNumberInRange(1, 10) : null,
    }));

    const credits = titles.flatMap((title) => [...Array(getRandomNumberInRange(5, 10))].map((_, i) => ({
      id: i + 1 + title.id * 10,
      title_id: title.id,
  ...
  function generateAndDownloadData() {
    ...
    const blob = new Blob([csvTitlesData, csvCreditsData], { type: 'text/csv' });
  ...
  return (
    <div className="App">
      <button onClick={generateAndDownloadData}>Perform synthetic data generation</button>
    </div>
  );
}
